## FRONTENT: [ПЕРЕЙТИ](https://github.com/NikitaVologin/DomovoiFrontend)
...
# Описание 1. Структура проекта:
Были использованы следующие определения: DDD (Модели анемичные), Clean Architecture.


Вся логика взаимодействия с объявлениями изолирована в AnnouncementService (Сервис объявлений), при необходимости добавления новой отвественности - использование декораторов.


Каждый из слоёв разделены при помощи абстракий (Interface).

![](images/projectStructure.png)

# Описание 2. Работа с маппингом запрос в сущности:
Сервис объявлений - используется один для всех сущностей Недвижимости.

Для ибзежения проблемы расширения кода в плане взаимодействия с каждой сущностью, был создан механизм маппинга.

Сбор сопоставления и их принятие запросов и сущностей производятся в MappingService (разделены абстракций, чтобы вместо библиотеки Automapper при нужде можно было изменить на другую).

IMapTwoSide<T> - где T, сущность к которой идёт сопоставление.
![](images/mappingStrategy.png)

# Описание 3. Конвейер декорации.
Для добавление дополнительной обязанности на сервис объявлений (либо другой), необходимо создать декоратор, расширяющий обязанности существующего сервиса.

ВАЖНО: порядок следования декораторов - важен

![](images/serviceConveyor.png)
